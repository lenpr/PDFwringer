...
- **Source/dest guard**: All services that take both source and destination URLs compare filesystem identity (including links and case aliases) before writing, throwing `PDFwringerError.sourceEqualsDestination` for the same file.
- **PDF permissions**: Derived and mutating workflows enforce the source document's copy, change, or assembly permissions through `PDFPermissionPolicy`; restricted encrypted PDFs fail without publishing output.
